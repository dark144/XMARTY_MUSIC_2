...
async def playlist(client, message):
    ...
    if not queue:
        await message.reply_text("ρℓαγєя ιѕ ι∂ℓє")
    temp = []
    for t in queue:
        temp.append(t)
    now_playing = temp[0][0]
    by = temp[0][1].mention(style="md")
    msg = "**иοω ρℓαγιиg** in {}".format(message.chat.title)
    msg += "\n- " + now_playing
    msg += "\n- яєգ ϐγ " + by
    temp.pop(0)
    if temp:
...
async def settings(client, message):
    ...
    if stats:
        ...
        await message.reply("иο νϲ ιиѕταиϲєѕ яυииιиg ιи τнιѕ ϲнατ")
...
async def play(_, message: Message):
    ...
    if audio:
        ...
        title = file_name
        thumb_name = "https://telegra.ph/file/3aa55e7efe0fabf05da4e.jpg"
        ...
        for administrator in administrators:
            if administrator == message_.from_user.id:
                ...
                try:
                    ...
                    await lel.edit(
                        f"<b>🔴 Flood Wait Error 🔴 \nUser {user.first_name} couldn't join your group due to heavy requests for userbot! Make sure user is not banned in group."
                        "\n\nOr manually add Bot to your Group and try again manually request here @Xmarty_Support with your group link</b>",
                    )
    try:
        await USER.get_chat(chid)
        # lmoa = await client.get_chat_member(chid,wew)
        ...
        await lel.edit(
            "<i> helper Userbot not in this chat, Ask admin to send /play command for first time or add assistant manually request here @warmusicSupport with your group link</i>"
        )
        return
